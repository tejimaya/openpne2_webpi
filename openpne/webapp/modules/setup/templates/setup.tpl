...
セットアップをやり直したい場合はデータベースを空にしてからこのページへアクセスしてください。<br>
通常の OpenPNE 2 のセットアップでは install-2.14-create_tables.sql と install-2.14-insert_data.sql
の実行及び、config.php の設定が必要でしたが、Web Platform Installer (WebPI) を用いたインストールでは必要ありません。
既に WebPI が自動的に実行しています。</p>
