...
({assign var="page_name" value="使用設定"})
...
<table class="contents" cellpadding="0" cellspacing="0" border="0">
   ...
   ({ext_include file="inc_menu_adminPortalConfig.tpl"})
       <td class="detail">
           ...
           <h3>({$page_name})</h3>
           <p>
           ログインページとして、「SNSポータル」を使用するかどうか選択してください。<br />
           「SNSポータル」を使用しない場合、 [ デザイン：スキン画像変更 ] の「ログイン画面」がログインページとして使用されます。
           </p>

           <p class="caution" id="c01">
           ※現在外部ログインページを使用している場合、ここで「使用する」を選択すると、SNS設定の[ 外部ログインページURL ]が上書きされますのでご注意ください。
           </p>

           <form action="./index.php" method="post">
           <input type="hidden" name="m" value="({$module_name})" />
           <input type="hidden" name="a" value="do_({$hash_tbl->hash('edit_portal_use','do')})" />
