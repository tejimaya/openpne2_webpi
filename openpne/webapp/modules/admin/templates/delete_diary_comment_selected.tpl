...
({foreach from=$diary_comment_list item=diary_comment})
...
<input type="hidden" name="a" value="do_({$hash_tbl->hash('delete_kakikomi_c_diary_comment_selected','do')})" />
<input type="hidden" name="target_c_diary_comment_ids" value="({$target_c_diary_comment_ids})" />
